...
function main(params) {
	...
	var servoHolderHeight = 13.5; //default: 11.5 - This will determine the height of the front bumper and hood too.
	var hoodHeightAdd = 1; //Raise the height of the hood, and so of the top part and stretch the lateral parts
	var lateralHeightAdd = 0; //Raise the height of the lateral parts, on top of the hoodHeightAdd (used for refine)
	...
	var steeringOutside = 2; //Move the steering outside;
	var steeringFrontMargin = -0.5; //Move the steering away from the bumper
	var steeringRodZAdjust = -1.5; //Adjust the height of the steering rod from the bottom (where the axle is connected and where the servo is connected)
	...
	var frontWheelsDiameter = 16;
	var rearWheelsDiameter = 18;
	var wheelHeight = 6;
	...
	var rearWheelsPaddingY = -4; //Move the rear wheels (and gears) on the Y axe
	...
	var	motorMarginZ = 8.2; //From the bottom of the chip (including chip layer)
	
	var gearTolerance = 0.5;
	...
	var steering = drawSteeringRod(axleTightHoles, steeringLength, servoHolderHeight, steeringRodZAdjust).setColorName("green");
	...
	if(SHOW_FRONT_BUMPER) {
		...
		//Top screw:
		bumper = bumper.subtract(car.properties.frontHoleFront.scaleSizeAnchored([screwDiameter, screwDiameter, 15], 0 ,0 ,-1));
	...
	if(SHOW_STEERING) {
		...
		var steeringBar = drawSteeringBar(steeringRight.properties.steeringPivotHole.getMiddleCoord("y") - steering.properties.steeringPivotHole.getMiddleCoord("y")).rotateZ(90);
	...
	if(SHOW_SERVO_HOLDER) {
		...
		var servoHolder = drawServoHolder(servoHolderHeight);
		...
		bottom = bottom.union(servoHolder);
	...
	if(SHOW_HOOD) {
		...
		//Screw:
		hood = hood.subtract(car.properties.frontHoleFront.scaleSizeAnchored([1.5, 1.5, 15], 0 ,0 ,-1));		
    ...
    if(SHOW_LR_BODY_PARTS) {
		...
		bodyLeft = bodyLeft.deform3(6, -2, -4, carLateralCutStart, carLateralCutEnd); //Deform
		...
		screwSupport = screwSupport.translate([0+11, screwSupport.getSize("y"), +plateHeight]);
	...
	if(SHOW_REAR_BOTTOM) {
		...
		var rearBottom = drawRearBottom(carWidth, carLength, rearBottomStructPaddingX, rearWheelsPaddingX, chipHeight /* <- it's placed on the plate */, rearWheelsPaddingY);
		...
		//Bearings Holder:
		var bearingsHolder = drawBearingsHolder(0.5 + abs(rearBottom.properties.rightBearing.getMiddleCoord("y") - rearBottom.properties.leftBearing.getMiddleCoord("y")), rearBottom.properties.leftBearing.getSize("z") - plateHeight - bearingDiameter/2).rotateZ(90);
		...
		if(SHOW_LR_BODY_PARTS) {
			...
			car.properties.bodyRight = bodyRight = car.properties.bodyRight.subtract(rearBottom.properties.wheelGearSpace
						/*fix: .translate([0, 2.5, 0])//<- TODO: fix to avoid STL Errors */
			);
		}
	...
	//*Top part:
	if(SHOW_REAR_TOP) {
		var rearTop = drawRearTop(car.properties.hood.getSize("y") + topWidthAdd, carLength );
		
		//Align to hood:
		var rearTop = rearTop.alignToAdjacent(car.properties.hood, -1, 0, -1).translate([-bumperTopOverlapX, 0, -bumperTopOverlapZ]); //Align to hood
	...
	if(SHOW_SERVO_HOLDER && SHOW_REAR_BOTTOM) {
		var chipHolderFront = drawCube([0,0,0], [1, servoHolder.getSize("y"), bottom.properties.leftBearing.getSize("z") - plateHeight]).setColorName("red");
		var chipHolderAlign = chipHolderFront.getAlignToAdjacent(servoHolder, -1, 0, 0);
		...
		var chipHolderDentFront = drawRoof().scaleSize([0, carWidth, 2]).alignToAdjacent(chipHolderFront, 0, 0, -1).translate([0, 0 ,0]);
		
		var chipHolderDentRear = chipHolderDentFront.mirroredY();
		var chipHolderDentRearMove = chipHolderDentRear.getAlignToAdjacent(bottom.properties.rearGearBox, 1 ,0,0);
		chipHolderDentRear = chipHolderDentRear.translate([chipHolderDentRearMove[0]+1, 0, 0]);		
		
		bottom = bottom.union(chipHolderFront).subtract(chipHolderDentFront);//.subtract(chipHolderDentRear);
		car = car.union(chipHolderFront).subtract(chipHolderDentFront);//.subtract(chipHolderDentRear);
	}
	...
	*/
	...
	if(!showPrintPlate) {
		...
		if(SHOW_LR_BODY_PARTS) {
			...
			carLie = carLie.union(car.properties.bodyRight.translate([-10, -50, -1*car.properties.bodyRight.getBounds()[0]["z"] ]));
		}
		if(SHOW_LR_BODY_PARTS) carLie = carLie.union(car.properties.bodyLeft.lieFlat().translate([0, -30, 0]));
		if(SHOW_HOOD) carLie = carLie.union(car.properties.hood.lieFlat().translate([20, 35, -0.1 /* <- fix strange error after the repair by netfabb */]));
		if(SHOW_STEERING) {
			carLie = carLie.union(car.properties.steeringRight.rotateZ(180).lieFlat().translate([-10, 25, 0]));
			carLie = carLie.union(car.properties.steeringLeft.lieFlat().translate([-10, 40, 0]));
			...
			carLie = carLie.union(car.properties.steeringBar.lieFlat().translate([-45, 50, 0]));
		}
		if(SHOW_REAR_TOP) {
